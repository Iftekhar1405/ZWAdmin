...
export default function Editbookings({ isOpen, onClose, data }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/bookings/${data._id}`, // Update endpoint
        formState
      );
      SuccessAlert("bookings updated successfully!");
      onClose(); // Close the modal after success
    } catch (error) {
      console.error("Error updating bookings:", error);
      ErrorAlert("Failed to update bookings");
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size={"5xl"}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Edit bookings</ModalHeader>
        <ModalCloseButton />
